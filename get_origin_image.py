...
wrong_folder = '/Users/waittim/Desktop/Draw_yolo/wrong/'  #错判图片文件夹wrong路径
...
def make_imageName_file_list_txt(wrong_folder, text_image_name_list_file_path):
...
if __name__ == '__main__':           # 只有在文件作为脚本文件直接执行时才执行下面代码  
    ...
    make_imageName_file_list_txt(wrong_folder, text_image_name_list_file_path) #执行写入txt函数

    image_names = open(text_image_name_list_file_path).read().strip().split() #得到图片名字不带后缀

    image_total = 0
    for image_name in image_names: #例遍图片名称
        box_num = get_image(image_name,raw_images_folder,save_images_folder)#对图片画框
        image_total += 1
